...
func test_health_bounds_invariant() -> void:
	assert_property("Health stays in [0, max_health]", func(seed: int) -> Dictionary:
		var rng := RandomNumberGenerator.new()
		rng.seed = seed
		
		# Reset health to random starting value
		var start_health := random_int(rng, 0, health.get_max_health())
		health._data.current_health = start_health
		health._is_alive = start_health > 0
...
func test_health_signal_emission() -> void:
	assert_property("Health signals emit correctly", func(seed: int) -> Dictionary:
		var rng := RandomNumberGenerator.new()
		rng.seed = seed
		...
		var signals_correct := (
			health_changed_count == expected_health_changed and
		...
		return {
			...
			"reason": "Expected health_changed=%d (got %d), died=%d (got %d)" % [
				expected_health_changed, health_changed_count,
				expected_died, died_count
			]
		}
	)
